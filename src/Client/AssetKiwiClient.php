...
class AssetKiwiClient {
   ...
   * In per-user OAuth mode the current user's personal access token is
   * preferred, falling back to the shared token if no personal token exists.
   */
  protected function resolveApiToken(): ?string {
    ...
    if ($uid) {
      return $this->oauthManager->getToken((int) $uid) ?: $this->apiToken;
    }
